...
def search_around_poly(image):
    ...
    if len(leftx) == 0 or len(rightx) == 0:
        out_img = np.dstack((image, image, image)) * 255
        return out_img, 0, 0, None, None, None
    ...
    out_img = cv2.fillPoly(out_img, np.int32([points]), (0, 200, 255))
...
st.write("Sliding window and polynomial lane detection.")
...
if vid is not None:
    ...
    result_placeholder = col2.empty() if show_binary else col3.empty()
    ...
    while vid.isOpened():
        ...
        if not ret:
            break
        binary_img = preprocess_frame(frame)

        if show_windows:
            leftx, lefty, rightx, righty, window_img = find_lane_pixels(binary_img)
            window_img_rgb = cv2.cvtColor(window_img, cv2.COLOR_BGR2RGB)
        else:
            window_img_rgb = None

        lane_img, left_curv, right_curv, left_fitx, right_fitx, ploty = search_around_poly(binary_img)
        ...
        if show_binary:
            binary_placeholder.image(cv2.cvtColor(binary_img, cv2.COLOR_GRAY2RGB), channels="RGB")
            result_placeholder.image(lane_img_rgb, channels="RGB")
        else:
            result_placeholder.image(lane_img_rgb, channels="RGB")
        if show_windows and window_img_rgb is not None:
            st.image(window_img_rgb, channels="RGB", caption="Sliding Windows")
        steering_placeholder.image(cv2.cvtColor(steering_wheel, cv2.COLOR_BGR2RGB), channels="RGB")

        avg_curv = (left_curv + right_curv)/2 if left_curv>0 and right_curv>0 else 0
        angle_placeholder.write(f"**Steering Angle:** {angle:.1f}°")
        curvature_placeholder.write(f"**Lane Curvature:** {avg_curv:.0f} m")
    ...
    st.info("Upload a video or use sample video to begin.")
